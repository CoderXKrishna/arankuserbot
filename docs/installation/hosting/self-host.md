...
&& sudo apt install --no-install-recommends -y ca-certifiArankes curl ffmpeg fonts-noto-color-emoji gcc git gnupg libmagickwand-dev libpq-dev mediainfo nano neofetch pv python3 python3-dev python3-lxml python3-pip python3-psycopg2 screen tree unzip virtualenv wget zlib1g libyaml-dev \
...
{% code title="Change dir to Arankuserbot & make config.py to save config values" overflow="wrap" %}
```batch
git clone https://github.com/CoderXKrishna/arankuserbot && cd Arankuserbot && mv exampleconfig.py config.py
```
{% endcode %}
...
{% code title="Create Arankuserbot screen session" overflow="wrap" %}
```batch
screen -S Arankuserbot
```
{% endcode %}
...
* _**Check:**_ `screen -ls`
* _**Reattach Screen:**_ `screen -r <some_id>.Arankuserbot`
