...
from ..helpers.utils import _arankutils, _format, install_pip, reply_id
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        mod._arankutils = _arankutils
        mod.edit_delete = edit_delete
        mod.install_pip = install_pip
        mod.parse_pre = _format.parse_pre
        mod.edit_or_reply = edit_or_reply
        mod.tgbot = arankub.tgbot
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    arankub.remove_event_handler(i)
    ...
    try:
        name = f"userbot.plugins.{shortname}"
        for i in reversed(range(len(arankub._event_builders))):
            ev, cb = arankub._event_builders[i]
            if cb.__module__ == name:
                del arankub._event_builders[i]
