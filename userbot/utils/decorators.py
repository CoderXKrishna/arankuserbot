    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                arankreg = f"^{Config.COMMAND_HAND_LER}"
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                arankreg = f"^\\{Config.COMMAND_HAND_LER}"
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(arankreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                arankreg = f"^{Config.SUDO_COMMAND_HAND_LER}"
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[here](https://t.me/arankuserbot_support)"
            text = "**arankUserbot Error report**\n\n" + "If you wanna you can report it"
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            arankub.add_event_handler(func, MessageEdited(**args))
        arankub.add_event_handler(func, NewMessage(**args))
        try:
