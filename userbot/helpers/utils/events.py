...
async def get_chatinfo(event, match, arankevent):
    ...
    try:
        ...
        try:
            ...
            await arankevent.edit("`Invalid channel/group`")
            return None
            ...
            await arankevent.edit("`The given Channel or Supergroup doesn't exist`")
            return None
        except (TypeError, ValueError):
            await arankevent.edit("**Error:**\n__Can't fetch the chat__")
            return None
    return chat_info
    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if previous_message.from_id is None:
                if not noedits:
                    await edit_delete(arankevent, "`Well that's an anonymous admin !`")
                return None, None
        ...
        if not args:
            if not noedits:
                await edit_delete(
                    arankevent, "`Pass the user's username, id or reply!`", 5
                )
            return None, None
    except Exception as e:
        LOGS.error(str(e))
    if not noedits:
        await edit_delete(arankevent, "__Couldn't fetch user to proceed further.__")
    return None, None


async def checking(arankub):
    arank_c = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    with contextlib.suppress(BaseException):
        arank_channel = Get(arank_c)
        await arankub(arank_channel)
