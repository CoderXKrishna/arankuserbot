...
async def generate_dalle_image(text, reply, event, flag=None):
    ...
    arankevent = await edit_or_reply(event, "__Generating image...__")
    try:
        if flag:
        ...
        await edit_delete(arankevent, f"Error generating image: {str(e)}")
        return None, None
    ...
    for i, media in enumerate(response["data"], 1):
        ...
        photos.append(photo)
        captions.append("")
        await edit_or_reply(arankevent, f"__📥 Downloaded : {i}/{limit}__")

    captions[-1] = f"**➥ Query :-** `{text.title()}`"
    await edit_or_reply(arankevent, "__Uploading...__")
