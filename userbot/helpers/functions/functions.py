...
async def post_to_telegraph(
    page_title,
    html_format_content,
    auth_name="arankUserbot",
    auth_url="https://t.me/arankuserbot17",
):
...
# ----------------------------------------------## Media ##-----------------------------------------------------------#
async def age_verifiarankion(event, reply_to_id):
    ALLOW_NSFW = gvarstatus("ALLOW_NSFW") or "False"
    if ALLOW_NSFW.lower() == "true":
        return False
    results = await event.client.inline_query(
        Config.TG_BOT_USERNAME, "age_verifiarankion_alert"
    )
...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    arank = await sticcers[c_lick].click("me", hide_via=True)
    if arank:
        await borg.send_file(int(chat_id), arank, reply_to=reply_to_id)
        await arank.delete()


async def make_inline(text, borg, chat_id, reply_to_id):
    arankinput = f"Inline buttons {text}"
    results = await borg.inline_query(Config.TG_BOT_USERNAME, arankinput)
...
def higlighted_text(
    ...
    for pic_no, list_text in enumerate(texts):
        ...
        output_img = f"./temp/arank{pic_no}.jpg"
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    arank = await sticcers[0].click("me", hide_via=True)
    if arank:
        await bot.send_file(int(chat_id), arank, reply_to=reply_to_id)
        await arank.delete()
