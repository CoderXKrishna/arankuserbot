...
async def admin_groups(arankub):
    arankgroups = []
    async for dialog in arankub.iter_dialogs():
...
async def extract_time(arank, time_val):
    if any(time_val.endswith(unit) for unit in ("s", "m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]  # type: str
        if not time_num.isdigit():
            await arank.edit("Invalid time amount specified.")
            return None
        if unit == "s":
