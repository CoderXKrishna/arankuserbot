    ...
    if message:
        try:
            ...
            if message.sticker:
                ...
                if mime == "appliarankion/x-tgsticker":
...
async def fileinfo(file):
    ...
    arank_json = json.loads(x)["media"]["track"]
    dic = {
        "path": file,
        "size": int(arank_json[0]["FileSize"]),
        "extension": arank_json[0]["FileExtension"],
        "type": "None",
        "format": "None",
        "audio": "None",
    }
    with contextlib.suppress(IndexError, KeyError):
        dic["format"] = arank_json[0]["Format"]
        dic["type"] = arank_json[1]["@type"]
        if "ImageCount" not in arank_json[0]:
            dic["duration"] = int(float(arank_json[0]["Duration"]))
            dic["bitrate"] = int(arank_json[0]["OverallBitRate"]) // 1000
        dic["height"] = int(arank_json[1]["Height"])
        dic["width"] = int(arank_json[1]["Width"])
        dic["audio"] = "Present" if arank_json[0]["AudioCount"] else "None"
    return dic
