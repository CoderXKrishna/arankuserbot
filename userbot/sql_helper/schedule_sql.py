...
class arankScheduler(BASE):
    __tablename__ = "arankscheduler"
...
arankScheduler.__table__.create(checkfirst=True)
...
def get_messages_to_send():
    ...
    result = (
        SESSION.query(arankScheduler)
        .filter(arankScheduler.scheduled_time == current_time)
        .all()
    )
    SESSION.close()
    return result
...
def get_all_messages():
    messages = SESSION.query(arankScheduler).all()
    SESSION.close()
    return messages
...
def delete_message_by_id(id):
    message = SESSION.query(arankScheduler).filter_by(id=id).first()
...
def delete_all_messages():
    SESSION.query(arankScheduler).delete()
    SESSION.commit()
    SESSION.close()
...
async def send_message(recipient, message):
    with contextlib.suppress(Exception):
        getmsg = await arankub.get_messages(
            int(message["chat"]), ids=int(message["msg_id"])
        )
        await arankub.send_message(int(recipient), getmsg)
