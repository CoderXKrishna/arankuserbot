...
class arank_GlobalCollection_Json(BASE):
    __tablename__ = "arank_globalcollectionjson"
...
arank_GlobalCollection_Json.__table__.create(checkfirst=True)


def get_collection(keywoard):
    try:
        return SESSION.query(arank_GlobalCollection_Json).get(keywoard)
    finally:
        SESSION.close()
...
def add_collection(keywoard, json, njson=None):
    ...
    if to_check := get_collection(keywoard):
        keyword_items = SESSION.query(arank_GlobalCollection_Json).get(keywoard)
        SESSION.delete(keyword_items)
    keyword_items = arank_GlobalCollection_Json(keywoard, json, njson)
...
def get_collections():
    try:
        return SESSION.query(arank_GlobalCollection_Json).all()
    finally:
        SESSION.close()
