...
class arankGloballist(BASE):
    __tablename__ = "arankglobal_list"
    ...
    def __repr__(self):
        return f"<arank global values '{self.group_id}' for {self.keywoard}>"

    def __eq__(self, other):
        return (
            isinstance(other, arankGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


arankGloballist.__table__.create(checkfirst=True)

arankGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with arankGLOBALLIST_INSERTION_LOCK:
        broadcast_group = arankGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with arankGLOBALLIST_INSERTION_LOCK:
        if broadcast_group := SESSION.query(arankGloballist).get(
...
def is_in_list(keywoard, group_id):
    with arankGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(arankGloballist).get((keywoard, str(group_id)))
...
def del_keyword_list(keywoard):
    with arankGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(arankGloballist.keywoard)
            .filter(arankGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(arankGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(arankGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(arankGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(arankGloballist).all()
        for x in all_groups:
