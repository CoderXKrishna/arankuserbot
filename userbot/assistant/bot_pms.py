...
async def check_bot_started_users(user, event):
    ...
    if check is None:
        ...
        notifiarankion = f"👤 {_format.mentionuser(user.first_name , user.id)} has started me.\
                \n**ID: **`{user.id}`\
                \n**Name: **{get_display_name(user)}"
    else:
        start_date = check.date
        notifiarankion = f"👤 {_format.mentionuser(user.first_name , user.id)} has restarted me.\
                \n**ID: **`{user.id}`\
                \n**Name: **{get_display_name(user)}"
    try:
        ...
        LOGS.error(str(e))
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, notifiarankion)
    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"Hey! 👤{mention},\
                        ...
                        \n\nPowered by [arankuserbot](https://t.me/arankuserbot)"
        buttons = [
            (
                Button.url("Repo", "https://github.com/CoderXKrishna/arankuserbot"),
                Button.url(
                    "Deploy",
                    "https://github.com/CoderXKrishna/nekopack",
                ),
            )
        ]
    else:
...
@arankub.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
@arankub.bot_cmd(pattern="^/uinfo$", from_users=Config.OWNER_ID)
...
async def send_flood_alert(user_) -> None:  # sourcery skip: low-code-quality
    ...
    if FloodConfig.ALERT and (user_.id in FloodConfig.ALERT.keys()):
        ...
        try:
            ...
            if BOTLOG:
                await arankub.tgbot.send_message(
                    BOTLOG_CHATID,
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await arankub.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await arankub.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await arankub.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await arankub.tgbot.get_entity(BOTLOG_CHATID)
            await arankub.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await arankub.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@arankub.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await arankub.get_entity(user_id)
    except Exception as e:
...
@arankub.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@arankub.bot_cmd(incoming=True, func=lambda e: e.is_private)
@arankub.bot_cmd(edited=True, func=lambda e: e.is_private)
