...
@arankub.arank_cmd(
    pattern="iytdl(?:\s|$)([\s\S]*)",
    command=("iytdl", plugin_arankegory),
    info={
...
async def iytdl_inline(event):
    ...
    arankevent = await edit_or_reply(event, f"🔎 Searching Youtube for: `'{input_url}'`")
    flag = True
    cout = 0
    results = None
    while flag:
    ...
    if results:
        ...
        await arankevent.edit("`Sorry!. Can't find any results`")
