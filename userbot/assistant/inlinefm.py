...
# Special Credit: Krishna https://t.me/itsz_krish_babess (Krishna)
...
from userbot.helpers.utils import _arankutils
...
# BACK
@arankub.tgbot.on(CallbackQuery(pattern="fmback"))
@check_owner
...
# UP
@arankub.tgbot.on(CallbackQuery(pattern="fmup_(.*)"))
@check_owner
async def up(event):
...
# DOWN
@arankub.tgbot.on(CallbackQuery(pattern="fmdown_(.*)"))
@check_owner
...
# FORTH
@arankub.tgbot.on(CallbackQuery(pattern="fmforth_(.*)"))
@check_owner
...
# REMOVE
@arankub.tgbot.on(CallbackQuery(pattern="fmrem_(.*)"))
@check_owner
async def remove(event):
    ...
    await asyncio.sleep(1)
    await event.edit(msg, buttons=buttons)
    await _arankutils.runcmd(f"rm -rf '{rpath}'")
    await event.answer(f"{rpath} removed successfully...")


# SEND
@arankub.tgbot.on(CallbackQuery(pattern="fmsend"))
@check_owner
...
# CUT
@arankub.tgbot.on(CallbackQuery(pattern="fmcut_(.*)"))
@check_owner
async def cut(event):
...
# COPY
@arankub.tgbot.on(CallbackQuery(pattern="fmcopy_(.*)"))
@check_owner
...
# PASTE
@arankub.tgbot.on(CallbackQuery(pattern="fmpaste_(.*)"))
@check_owner
