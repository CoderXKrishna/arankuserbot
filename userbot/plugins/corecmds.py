...
from . import CMD_HELP, CMD_LIST, SUDO_LIST, arankub, edit_delete, edit_or_reply, reply_id
...
def plug_checker(plugin):
    ...
    if not os.path.exists(plug_path):
        plug_path = f"./badarankext/{plugin}.py"
    if not os.path.exists(plug_path):
        plug_path = f"./arankvc/{plugin}.py"
    return plug_path
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "description": "Reply to any external plugin(supported by arank) to install it in your bot.",
        "usage": "{tr}install",
    },
)
...
@arankub.arank_cmd(
    ...
    info={
        "header": "To send the log of arankub",
