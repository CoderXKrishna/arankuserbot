...
@arankub.arank_cmd(
    ...
    info={
        "header": "To show system specifiarankion.",
        "usage": "{tr}spc",
    },
)
async def psu(event):
    "shows system specifiarankion"
...
async def cpu(event):
    ...
    cmd = "arank /proc/cpuinfo | grep 'model name'"
    o = (await _arankutils.runcmd(cmd))[0]
    await edit_or_reply(
        event, f"**[arank's](tg://need_update_for_some_feature/) CPU Model:**\n{o}"
    )
...
async def sysdetails(sysd):
    ...
    arankevent = await edit_or_reply(sysd, "`Fetching system information.`")
    ...
    a, b, c, d = await _arankutils.runcmd(neo)
    result = str(a) + str(b)
    await edit_or_reply(arankevent, f"**Neofetch Result:** `{result}`")
