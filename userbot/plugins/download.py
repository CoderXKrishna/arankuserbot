...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    if reply:
        ...
        if downloader.isSuccessful():
            await mone.edit(
                f"**•  Downloaded in {ms} seconds.**\n**•  Downloaded file loarankion :- ** `{os.path.relpath(downloaded_file_name,os.getcwd())}`"
            )
        else:
...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    loarankion = os.path.join(pwd, input_str)
    if not os.path.isdir(loarankion):
        os.makedirs(loarankion)
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(
            event,
    ...
    for attr in getattr(reply.document, "attributes", []):
        if isinstance(attr, types.DocumentAttributeFilename):
            name = attr.file_name
    path = pathlib.Path(os.path.join(loarankion, name))
    ext = get_extension(reply.document)
    if path and not path.suffix and ext:
        path = path.with_suffix(ext)
    if name == NAME:
    ...
    if path and path.exists():
        ...
        file_name = loarankion / path.with_suffix(ext)
    elif path:
        file_name = path
    else:
        file_name = loarankion / name
    file_name.parent.mkdir(parents=True, exist_ok=True)
    c_time = time.time()
    if (
        not reply.document
