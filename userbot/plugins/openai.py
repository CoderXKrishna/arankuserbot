...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}gpt write a paragraph on arank",
...
async def gpt_response_with_prompt(event):
    ...
    arankevent = await edit_or_reply(event, "__Generating answer...__")
    gpt_response = generate_gpt_response(text, chat_id)
    await edit_or_reply(arankevent, gpt_response)
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}dalle arank & dog riding bicycle",
            "{tr}dalle -n 3",
            "{tr}dalle -s 512",
            "{tr}dalle -e write arank on it <reply to an Image>.",
            "{tr}dalle -v",
        ],
