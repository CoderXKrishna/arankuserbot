...
async def rayso_by_pro_odi(event):  # By @feelded
    ...
    if checker and (checker[0].lower() in THEMES or checker[0].lower() == "random"):
        ...
        if checker[0] == query and not rquery:
            return await edit_delete(arankevent, f"`Theme changed to {query.title()}.`")
    ...
    if checker and checker[0].lower() in MODES:
        ...
        if checker[0] == query and not rquery:
            return await edit_delete(
                arankevent, f"`Theme Mode changed to {query.title()}.`"
            )
        query = checker[1] if len(checker) > 1 else None
    ...
    if query == "-l":
        ...
        for i, each in enumerate(THEMES, start=2):
            ALLTHEME += f"\n**{i}.**  `{each.title()}`"
        return await edit_delete(arankevent, ALLTHEME, 60)
    ...
    if query:
        ...
        if rquery.file and rquery.file.mime_type.startswith("text"):
            ...
            return await edit_delete(arankevent, "`Unsupported.`")
    else:
        return await edit_delete(arankevent, "`What should I do?`")

    # // Max size 30000 byte but that breaks thumb so making on 28000 byte
    text_list = text_chunk_list(text, 28000)
    user = (await arankub.get_me()).first_name
    for i, text in enumerate(text_list, start=1):
        await edit_or_reply(arankevent, f"**⏳ Pasting on image : {i}/{len(text_list)} **")
    ...
    await edit_or_reply(arankevent, "**📎 Uploading... **")
    captions[-1] = f"<i>➥ Generated by : <b>{hmention}</b></i>"
    await arankub.send_file(
        event.chat_id,
        files,
...
async def paste_img(event):
    ...
    d_file_name = None
    arankevent = await edit_or_reply(event, "`Pasting the text on image`")
...
async def paste_bin(event):
    ...
    arankevent = await edit_or_reply(event, "`pasting text to paste bin....`")
    ...
    try:
        ...
        await arankevent.edit(result, link_preview=False, parse_mode="html")
    except Exception as e:
        await edit_delete(arankevent, f"**Error while pasting text:**\n`{e}`")
...
async def get_dogbin_content(event):
    ...
    arankevent = await edit_or_reply(event, "`Getting Contents of pastebin.....`")
    ...
    if rawurl is None:
        ...
        elif "arankbin" in url:
            rawurl = f"http://arankbin.up.railway.app/raw/{fid[0]}"
        else:
    ...
    try:
        ...
        return await arankevent.edit(f"**Request timed out.**__{str(TimeoutErr)}__")
    ...
    await edit_or_reply(arankevent, reply_text)
...
async def _(event):
    ...
    try:
        ...
        return await edit_delete(arankevent, f"**Error while pasting text:**\n`{e}`")
    url = response["url"]
    chat = "@CorsaBot"
    await arankevent.edit("`Making instant view...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return await arankevent.edit("```Please unblock me (@CorsaBot) and try```")
        result = ""
        if response:
        ...
        await arankevent.edit(result, link_preview=True)
