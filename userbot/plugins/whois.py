...
async def _(event):
    ...
    if not replied_user:
        return
    arankevent = await edit_or_reply(event, "`Fetching userinfo wait....`")
    FullUser = (await event.client(GetFullUserRequest(replied_user.id))).full_user
    user_id = replied_user.id
...
""".format(
    ...
    await edit_or_reply(arankevent, caption)
...
async def who(event):
    ...
    if not replied_user:
        return
    arank = await edit_or_reply(event, "`Fetching userinfo wait....`")
    try:
        photo, caption = await fetch_info(replied_user, event)
    except (AttributeError, TypeError):
        return await edit_delete(arank, "`Could not fetch info of that user.`")
    ...
    try:
        ...
        await arank.edit(caption, parse_mode="html")
