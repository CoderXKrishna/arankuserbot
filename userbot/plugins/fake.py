...
async def _(event):
    ...
    user, rank = await get_user_from_event(event, arankevent)
    if not rank:
        rank = "Admin"
    if not user:
        return
    try:
        ...
        return await arankevent.edit("__I think you don't have permission to promote__")
    except Exception as e:
        return await edit_delete(arankevent, f"__{e}__", time=10)
