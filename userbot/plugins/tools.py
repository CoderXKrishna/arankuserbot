...
from ..helpers.utils import _arankutils, reply_id
...
async def currency(event):
    ...
    try:
        ...
        symbols = await AioHttp().get_raw(
            "https://raw.githubusercontent.com/CoderXKrishna/arankUserbot-Resources/master/Resources/Data/currency.py"
        )
...
async def scan(event):
    ...
    chat = "@VS_Robot"
    arankevent = await edit_or_reply(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        try:
            flag = await conv.send_message("/start")
        except YouBlockedUserError:
            await edit_or_reply(
                arankevent, "**Error:** Trying to unblock & retry, wait a sec..."
            )
            await arankub(unblock("VS_Robot"))
            flag = await conv.send_message("/start")
        ...
        if response1.text:
            ...
            await edit_delete(arankevent, f"**Please wait for {sec}s before retry**", 15)
        else:
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            ...
            await event.client.send_read_acknowledge(conv.chat_id)
            if not input_str:
                await edit_or_reply(arankevent, response3.text[30:])
            else:
...
async def parseqr(event):
    ...
    arankevent = await edit_or_reply(event, "`Decoding....`")
    ...
    t_response, e_response = (await _arankutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await edit_or_reply(arankevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await edit_or_reply(arankevent, f"**Failed to Decode:**\n`{result}`")
...
async def _(event):
    ...
    arankevent = await edit_or_reply(event, "...")
    start = datetime.now()
    input_str = event.pattern_match.group(1)
    ...
    try:
        ...
        return await arankevent.edit(str(e))
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(arankevent, f"Created BarCode in {ms} seconds")
...
async def _(event):
    ...
    arankevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
    ...
    if not r.ok:
        return await arankevent.edit(f"xkcd n.{xkcd_id} not found!")
    data = r.json()
    year = data.get("year")
...
Year: {}""".format(
    ...
    await arankevent.edit(output_str, link_preview=True)
