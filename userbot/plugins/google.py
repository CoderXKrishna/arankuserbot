...
from userbot import BOTLOG, BOTLOG_CHATID, Convert, arankub
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}gs arankuserbot",
            "{tr}gs -l6 arankuserbot",
            "{tr}gs -p2 arankuserbot",
            "{tr}gs -p2 -l7 arankuserbot",
        ],
    },
)
async def gsearch(q_event):
    "Google search command."
    arankevent = await edit_or_reply(q_event, "`searching........`")
...
async def reverse(event):
    ...
    if data["error"]:
        return await edit_delete(arankevent, data["error"])
    if data["lens"] is None:
        return await edit_delete(arankevent, "`Couldn't find any reverse data..`")
    ...
    if cmd == "grs":
        ...
        for checker, item in enumerate(data["image_set"], 1):
            ...
            try:
                image = await arankub.send_file(BOTLOG_CHATID, url, silent=True)
                if url.endswith(".gif"):
                    await unsavegif(event, image)
                    giflink = await arankub.get_msg_link(image)
                    gifstring += f'<b><a href="{giflink}">Gif{checker}</a></b>  '
                elif not url.endswith(".gif"):
                    imagelist.append(image.media)
                    captionlist.append("")
                    await image.delete()
                await edit_or_reply(arankevent, f"**📥 Downloaded : {checker}/{limit}**")
