...
from userbot import BOTLOG, BOTLOG_CHATID, arankub
...
async def group_fban(event):
    ...
    if errors != []:
        ...
        for txt in errors:
            success_report += f"\n☞ __{txt}__"
    await edit_or_reply(arankevent, success_report)
...
async def quote_search(event):  # sourcery no-metrics
    ...
    if get_collection("fedids") is not None:
        ...
        feds = {}
    if fedgroup == "-all":
        arankevent = await edit_or_reply(event, "`Adding all your feds to database...`")
        fedidstoadd = []
        async with event.client.conversation("@MissRose_bot") as conv:
            try:
                try:
                    ...
                    await arankub(unblock("MissRose_bot"))
                    await conv.send_message("/myfeds")
                ...
                if "can only" in response.text:
                    return await edit_delete(arankevent, f"__{response.text}__")
                ...
                await edit_delete(
                    arankevent, f"**Error while fecthing myfeds:**\n__{e}__", 10
                )
...
async def fetch_fedinfo(event):
    ...
    arankevent = await edit_or_reply(event, "`Fetching info about given fed...`")
    async with event.client.conversation(rose) as conv:
        try:
            try:
                ...
                await conv.send_message(f"/fedinfo {input_str}")
            response = await conv.get_response()
            await edit_or_reply(arankevent, response.text)
        except Exception as e:
            await edit_delete(
                arankevent, f"**Error while fecthing fedinfo:**\n__{e}__", 10
            )
        await event.client.send_read_acknowledge(conv.chat_id)
        conv.cancel()
...
async def fetch_fedinfo(event):
    ...
    arankevent = await edit_or_reply(event, "`Fetching admins list of given fed...`")
    async with event.client.conversation(rose) as conv:
        try:
            try:
...
async def myfeds_fedinfo(event):
    ...
    arankevent = await edit_or_reply(event, "`Fetching list of feds...`")
...
async def fstat_rose(event):
    ...
    arankevent = await edit_or_reply(event, "`Fetching fedstat from given deatils...`")
    user, fedid = await get_user_from_event(
        event, arankevent, secondgroup=True, noedits=True
    )
    if user is None:
        user = await event.client.get_me()
    if fedid is None:
    ...
    async with event.client.conversation(rose) as conv:
        try:
            ...
            if "Looks like" in response.message:
                ...
                await arankevent.delete()
                return
            await edit_or_reply(arankevent, result + response.text)
        except Exception as e:
            await edit_delete(
                arankevent, f"**Error while fecthing fedstat:**\n__{e}__", 10
            )
        await event.client.send_read_acknowledge(conv.chat_id)
        conv.cancel()
