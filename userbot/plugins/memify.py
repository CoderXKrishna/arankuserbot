...
from ..helpers import asciiart, arank_meeme, arank_meme, media_type
...
async def maccmd(event):  # sourcery no-metrics
    ...
    arankevent = await event.edit("__Adding frame for media....__")
    args = event.pattern_match.group(1)
    force = bool(args)
    try:
        imag = await Convert.to_image(
            arankevent, reply, dirct="./temp", file="pframe.png", noedits=True
        )
        if imag[1] is None:
            return await edit_delete(
...
async def memes(event):
    ...
    arankinput = event.pattern_match.group(2)
    reply = await event.get_reply_message()
    if not reply:
    ...
    meme_file = output[1]
    meme = os.path.join("./temp", "arankmeme.jpg")
    if gvarstatus("CNG_FONTS") is None:
        CNG_FONTS = "userbot/helpers/styles/impact.ttf"
    else:
        CNG_FONTS = gvarstatus("CNG_FONTS")
    if max(len(top), len(bottom)) < 21:
        await arank_meme(CNG_FONTS, top, bottom, meme_file, meme)
    else:
        await arank_meeme(top, bottom, CNG_FONTS, meme_file, meme)
    if cmd != "mmf":
        meme = (await Convert.to_sticker(event, meme, file="memes.webp", noedits=True))[
            1
        ]
    await event.client.send_file(
        event.chat_id, meme, reply_to=arankid, force_document=False
    )
...
async def lang(event):
    ...
    if input_str not in font_list:
        arankevent = await edit_or_reply(event, "`Give me a correct font name...`")
        await asyncio.sleep(1)
        await arankevent.edit(f"**Available Fonts names are here:-**\n\n{FONTS}")
    else:
...
async def memes(event):
    ...
    color1 = c_list[0]
    color2 = c_list[1]
    bgcolor = arankinput or "#080808"
    asciiart(meme_file, 0.3, 1.9, outputfile, color1, color2, bgcolor)
    await event.client.send_file(
        event.chat_id, outputfile, reply_to=arankid, force_document=False
    )
    await output[0].delete()
...
async def memes(event):
    ...
    await event.client.send_file(
        event.chat_id, outputfile, force_document=False, reply_to=arankid
    )
    await output[0].delete()
    for files in (outputfile, meme_file):
        if files and os.path.exists(files):
...
async def memes(event):
    ...
    arankinput = event.pattern_match.group(1)
    arankinput = int(arankinput) if arankinput else 50
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "`Reply to supported Media...`")
    san = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    ...
    try:
        await crop(meme_file, outputfile, arankinput)
    except Exception as e:
...
async def memes(event):
    ...
    if not arankinput:
        arankinput = "50"
    if ";" in str(arankinput):
        arankinput, colr = arankinput.split(";", 1)
    else:
        colr = 0
    ...
    try:
        await add_frame(meme_file, outputfile, arankinput, colr)
    except Exception as e:
        return await output[0].edit(f"`{e}`")
    try:
        await event.client.send_file(
