...
async def ban_user(chat_id, i, rights):
    try:
        await arankub(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
...
async def rm_deletedacc(show):  # sourcery no-metrics
    ...
    if con != "clean":
        ...
        if flag != " -r":
            ...
            arankadmin = await is_admin(show.client, show.chat_id, show.client.uid)
            if not arankadmin:
