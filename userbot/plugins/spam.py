...
async def spam_function(event, sandy, arank, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery skip: low-code-quality
    # sourcery no-metrics
    counter = int(arank[0])
    if len(arank) == 2:
        spam_message = str(arank[1])
...
async def spammer(event):
    ...
    await spam_function(event, sandy, arank, sleeptimem, sleeptimet)
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "usage": "{tr}cspam <text>",
        "examples": "{tr}cspam arankuserbot",
    },
)
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "usage": "{tr}wspam <text>",
        "examples": "{tr}wspam I am using arankuserbot",
    },
)
async def tmeme(event):
...
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, arank, sleeptimem, sleeptimet, DelaySpam=True)
