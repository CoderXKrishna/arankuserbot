...
#                           STRINGS                           #
# =========================================================== #
STAT_INDIarankION = "`Collecting stats, Wait man`"
...
async def stats(event):  # sourcery no-metrics # sourcery skip: low-code-quality
    ...
    arank = await edit_or_reply(event, STAT_INDIarankION)
...
async def full_stats(event):  # sourcery no-metrics # sourcery skip: low-code-quality
    ...
    arankcmd = event.pattern_match.group(2)
    arankevent = await edit_or_reply(event, STAT_INDIarankION)
    start_time = time.time()
    grp = []
    message = []
    async for dialog in event.client.iter_dialogs():
        ...
        if isinstance(entity, Channel) and entity.broadcast:
            if flag == "":
                if arankcmd == "c":
                    ...
                    output = CHANNELS_STR
                if (entity.creator or entity.admin_rights) and arankcmd == "ca":
                    grp.append(
                        f"<a href = https://t.me/c/{entity.id}/1>{entity.title}</a>"
                    )
                    output = CHANNELS_ADMINSTR
                if entity.creator and arankcmd == "co":
                    grp.append(
                        f"<a href = https://t.me/c/{entity.id}/1>{entity.title}</a>"
                    )
                ...
                with contextlib.suppress(AttributeError):
                    if entity.username and arankcmd == "c":
                        grp.append(
                    ...
                    if (
                        ...
                        output = CHANNELS_ADMINSTR
                    if (entity.creator and entity.username) and arankcmd == "co":
                        grp.append(
                            f"<a href = https://t.me/{entity.username}>{entity.title}</a>"
                        )
                        output = CHANNELS_OWNERSTR
            ...
            if flag == "":
                if arankcmd == "g":
                    ...
                    output = GROUPS_STR
                if (entity.creator or entity.admin_rights) and arankcmd == "ga":
                    grp.append(
                        f"<a href = https://t.me/c/{entity.id}/1>{entity.title}</a>"
                    )
                    output = GROUPS_ADMINSTR
                if entity.creator and arankcmd == "go":
                    grp.append(
                        f"<a href = https://t.me/c/{entity.id}/1>{entity.title}</a>"
                    )
                ...
                with contextlib.suppress(AttributeError):
                    if entity.username and arankcmd == "g":
                        grp.append(
                            f"<a href = https://t.me/{entity.username}>{entity.title}</a>"
                        )
                        output = GROUPS_STR
                    if (
                        ...
                        output = GROUPS_ADMINSTR
                    if (entity.creator and entity.username) and arankcmd == "go":
                        grp.append(
                            f"<a href = https://t.me/{entity.username}>{entity.title}</a>"
                        )
                        output = GROUPS_OWNERSTR
    ...
    await arankevent.edit(message[0], parse_mode="html")
    reply_to_msg = event.id
    if count > 1:
        for i in range(1, count):
            new_event = await arankub.send_message(
...
async def ustat(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await arankub(unblock("BRScan_bot"))
            purgeflag = await conv.send_message(f"/search {uid}")
        msg = ""
        chat_list = []
        msg_list = []
    ...
    if "user is not in my database" in chat_list[0]:
        return await edit_delete(arankevent, "`User not found in database!`")
    ...
    await arankevent.edit(msg_list[0])
    reply_to_msg = event.id
    if checker > 1:
