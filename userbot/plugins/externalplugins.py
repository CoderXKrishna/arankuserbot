...
from . import BOTLOG, BOTLOG_CHATID, arankub
...
if Config.PLUGIN_CHANNEL:
    ...
    async def install():
        documentss = await arankub.get_messages(
        ...
        for module in range(total):
            ...
            if os.path.exists(f"userbot/plugins/{plugin_name}"):
                return
            downloaded_file_name = await arankub.download_media(
                await arankub.get_messages(Config.PLUGIN_CHANNEL, ids=plugin_to_install),
                "userbot/plugins/",
            )
    ...
    arankub.loop.create_task(install())
