...
async def anilist_usersearch(event):
    ...
    arankevent = await edit_or_reply(event, "`Searching user profile in anilist...`")
    searchresult = await anilist_user(search_query)
    if len(searchresult) == 1:
        return await edit_or_reply(
            arankevent, f"**Error while searching user profile:**\n{searchresult[0]}"
        )
...
async def anilist(event):  # sourcery no-metrics
    ...
    arankevent = await edit_or_reply(event, "`Searching Anime..`")
    ...
    if not respone:
        return await edit_delete(arankevent, result)
    if len(result) == 0:
        return await edit_or_reply(
            arankevent, f"**Search query:** `{query}`\n**Result:** `No results found`"
        )
    input_str = result[0]["title"]["english"] or result[0]["title"]["romaji"]
    if myanime:
        result = await callAPI(input_str)
        msg = await formatJSON(result)
        await arankevent.edit(msg, link_preview=True)
        return
    if listview:
        ...
        for result in ani_data:
            ...
            i += 1
        await arankevent.edit(msg, parse_mode="html")
        return
    ...
    if image is None:
        await edit_or_reply(arankevent, caption, parse_mode="html")
        return
    try:
...
async def anilist(event):  # sourcery no-metrics
    ...
    arankevent = await edit_or_reply(event, "`Searching manga..`")
    match = match.replace("-s", "")
    listview = bool(listview)
    match = match.replace("-d", "")
    myanime = bool(myanime)
...
async def anilist(event):  # sourcery no-metrics
    ...
    arankevent = await edit_or_reply(event, "`Searching character..`")
    match = match.replace("-s", "")
    listview = bool(listview)
    query = match.strip()
...
async def whatanime(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"https://api.trace.moe/search?anilistInfo&url={quote_plus(arank)}"
        ) as raw_resp0:
