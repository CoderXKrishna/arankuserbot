...
async def sticklet(event):
    ...
    fontsize = 230
    json = requests.get(
        "https://raw.githubusercontent.com/CoderXKrishna/arankUserbot-Resources/master/Resources/StickerFun/resources.txt"
    ).json()
    ...
    image_stream.name = "arankuserbot.webp"
...
async def quby(event):
    ...
    if cmd == "b":
        ...
        await event.client.send_file(
            event.chat_id, arank, reply_to=reply_to_id, force_document=False
        )
    else:
...
async def doge(event):
    ...
    arank = (await Convert.to_sticker(event, file[0], file="doge.webp", noedits=True))[1]
    await event.client.send_file(
        event.chat_id, arank, reply_to=reply_to_id, force_document=False
    )
    await event.delete()
...
async def penguin(event):
    ...
    arank = (await Convert.to_sticker(event, file[0], file="penguin.webp", noedits=True))[
        1
    ]
    await event.client.send_file(
        event.chat_id, arank, reply_to=reply_to_id, force_document=False
...
async def gandhi(event):
    ...
    arank = (await Convert.to_sticker(event, file[0], file="gandhi.webp", noedits=True))[
        1
    ]
    await event.client.send_file(
        event.chat_id, arank, reply_to=reply_to_id, force_document=False
