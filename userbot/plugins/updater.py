...
from userbot import HEROKU_APP, UPSTREAM_REPO_URL, arankub
...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n`arankUSERBOT is`  **up-to-date**  `with`  "
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"do `{cmdhd}update deploy` to update the arankuserbot"
        )
...
async def upstream(event):
    ...
    off_repo = "https://github.com/CoderXKrishna/nekopack"
    os.chdir("/app")
    try:
        txt = (
            "`Oops.. Updater cannot continue due to "
...
@arankub.arank_cmd(
    pattern="(good|bad)arank$",
    command=("switch", plugin_category),
    info={
        "header": "To switch between goodarank & badarank(For extra nsfw and gali).",
        "usage": [
            "{tr}goodarank",
            "{tr}badarank",
        ],
...
async def variable(event):  # sourcery skip: low-code-quality
    "To switch between good & bad arank"
    switch = "BADarank"
    config = "config.py"
    ...
    if ENV:
        ...
        if cmd == "good":
            if BADarank:
                await edit_or_reply(
                    event, "`Changing badarank to goodarank wait for 2-3 minutes.`"
                )
                del heroku_var[switch]
                return
            await edit_delete(event, "`You already using Goodarank`", 6)
        else:
            if BADarank:
                return await edit_delete(event, "`You already using Badarank`", 6)
            await edit_or_reply(
                event, "`Changing goodarank to badarank wait for 2-3 minutes.`"
            )
        ...
        for i in configs:
            if switch in i:
                ...
                string += f"{i}"
        if cmd == "good":
            if match and not BADarank:
                arank = await edit_or_reply(
                    event, "`Changing badarank to goodarank wait for 2-3 minutes.`"
                )
                ...
                return await event.client.reload(arank)
            await edit_delete(event, "`You already using Goodarank`")
        elif cmd == "bad":
            if match and BADarank:
                return await edit_or_reply(event, "`You already using Badarank`")
