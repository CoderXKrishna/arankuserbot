...
async def noods(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    arankevent = await edit_or_reply(event, "🔍 `Searching...`")
    r = requests.get(
    ...
    if not walls:
        return await edit_delete(
            arankevent, f"**Can't find anything with** `{query}`", 10
        )
    i = count = 0
    piclist = []
    piclinks = []
    ...
    for x in walls:
        ...
        if "We are sorry," not in requests.get(a).text and a not in piclinks:
            await edit_or_reply(arankevent, "📥** Downloading...**")
            pic = await wall_download(a, query)
            if pic is None:
                return await edit_delete(
                    arankevent, "__Sorry i can't download wallpaper.__"
                )
            piclist.append(pic)
            piclinks.append(a)
            captionlist.append("")
            ...
            i += 1
        await edit_or_reply(
            arankevent, f"**📥 Downloaded : {count}/{limit}\n\n❌ Errors : {i}/5**"
        )
        if count == int(limit):
            break
        if i == 5:
            await edit_or_reply(arankevent, "`Max search error limit exceed..`")
    try:
        await edit_or_reply(arankevent, "`Sending...`")
