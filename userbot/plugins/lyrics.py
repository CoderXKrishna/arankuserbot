...
async def lyrics(event):  # sourcery no-metrics
    ...
    if "-" in query:
        ...
        arankevent = await edit_or_reply(event, f"`Searching lyrics for {songinfo}...`")
        lyrics = LyricsGen.lyrics(song, artist)[1]
        if lyrics is None:
            return await arankevent.edit(f"Song **{songinfo}** not found!")
        result = f"**Search query**: \n`{songinfo}`\n\n```{lyrics}```"
    else:
        arankevent = await edit_or_reply(event, f"`Searching lyrics for {query}...`")
    ...
    await edit_or_reply(arankevent, result)
