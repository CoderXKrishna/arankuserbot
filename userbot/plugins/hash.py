...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": ["{tr}hbase en arankuserbot", "{tr}hbase de Q2F0dXNlcmJvdA=="],
    },
)
    ...
    if event.pattern_match.group(1) == "en":
        if string:
            ...
            if mediatype is None:
                ...
                arankevent = await edit_or_reply(event, "`Encoding ...`")
                c_time = time.time()
        ...
        await edit_or_reply(
            arankevent, result, file_name="encodedfile.txt", caption="It's Encoded"
        )
    else:
        try:
            lething = str(
