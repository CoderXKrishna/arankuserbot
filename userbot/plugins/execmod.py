...
from ..helpers.utils import _arankutils, parse_pre, yaml_format
...
async def _(event):
    ...
    OUTPUT = f"**[arank's](tg://need_update_for_some_feature/) PLUGINS:**\n{o}"
...
async def _(event):
    ...
    o = (await _arankutils.runcmd(cmd))[0]
    OUTPUT = (
        f"**[arank's](tg://need_update_for_some_feature/) Environment Module:**\n\n\n{o}"
    )
    await edit_or_reply(event, OUTPUT)
