...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": "{tr}admins @arankuserbot_support",
    },
)
async def _(event):
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": "{tr}bots @arankuserbot_support",
    },
)
async def _(event):
    "To get list of bots."
...
async def get_users(show):
    ...
    arankevent = await edit_or_reply(show, "`getting users list wait...`  ")
    try:
        ...
        mentions += f" {str(e)}" + "\n"
    await edit_or_reply(arankevent, mentions)
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": "{tr}chatinfo @arankuserbot_support",
    },
)
async def info(event):
    "To get group information"
    arankevent = await edit_or_reply(event, "`Analysing the chat...`")
    match = event.pattern_match.group(1)
    chat = await get_chatinfo(event, match.strip(), arankevent)
    if not chat:
        return
    caption = await fetch_info(chat, event)
    try:
        await arankevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, f"**Error in chatinfo : **\n`{e}`"
            )
        await arankevent.edit("`An unexpected error has occurred.`")
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}grpstats @arankuserbot_support",
            "{tr}grpstats -q2000 @arankuserbot_support",
            "{tr}grpstats -l20 @arankuserbot_support",
            "{tr}grpstats -s @arankuserbotot",
            "{tr}grpstats -s -l20 -q2000 @arankuserbotot",
        ],
    },
)
async def grp_stat(event):  # sourcery skip: low-code-quality
    ...
    try:
        ...
        flag = None
    temp = {}
    chatinfo = await get_chatinfo(event, match.strip(), arankevent)
    if not chatinfo:
        return
    ...
    await arankevent.edit(string, parse_mode="html")
