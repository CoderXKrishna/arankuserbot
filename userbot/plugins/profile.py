...
async def _(event):
    ...
    try:
        ...
        if photo:
            await arankevent.edit("`now, Uploading to Telegram ...`")
            if photo.endswith((".mp4", ".MP4")):
                # https://t.me/tgbetachat/324694
                size = os.stat(photo).st_size
                if size > 2097152:
                    await arankevent.edit("`size must be less than 2 mb`")
                    os.remove(photo)
                    return
                arankpic = None
                arankvideo = await event.client.upload_file(photo)
            else:
                arankpic = await event.client.upload_file(photo)
                arankvideo = None
            try:
                await event.client(
                    functions.photos.UploadProfilePhotoRequest(
                        file=arankpic, video=arankvideo, video_start_ts=0.01
                    )
                )
            except Exception as e:
                ...
                await edit_or_reply(
                    arankevent, "`My profile picture was successfully changed`"
                )
    try:
        os.remove(photo)
    except Exception as e:
