...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# Special Credits: @itsz_krish_babess for batmanpfp and thorpfp
...
from . import BOTLOG, BOTLOG_CHATID, _arankutils, arankub, edit_delete, logging
...
async def autopicloop():
    ...
    if AUTOPICSTART and DEFAULT_PIC is None:
        if BOTLOG:
            return await arankub.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await arankub.upload_file(autophoto_path)
        try:
            await arankub(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await arankub.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await arankub(
                    functions.photos.DeletePhotosRequest(
                        await arankub.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await arankub(functions.photos.UploadProfilePhotoRequest(file))
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        img = Image.open(autophoto_path)
        drawn_text = ImageDraw.Draw(img)
        arank = str(base64.b64decode("dXNlcmJvdC9oZWxwZXJzL3N0eWxlcy9kaWdpdGFsLnR0Zg=="))[
            2:36
        ]
        fnt = ImageFont.truetype(arank, 200)
...
async def autoname_loop():
    ...
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await arankub(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await arankub(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        i += 1
        await arankub(functions.photos.UploadProfilePhotoRequest(file))
        await _arankutils.runcmd("rm -rf donottouch.jpg")
        await asyncio.sleep(CHANGE_TIME)
...
arankub.loop.create_task(autopfp_start())
arankub.loop.create_task(autopicloop())
arankub.loop.create_task(digitalpicloop())
arankub.loop.create_task(bloom_pfploop())
arankub.loop.create_task(autoname_loop())
arankub.loop.create_task(autobio_loop())
arankub.loop.create_task(custompfploop())
