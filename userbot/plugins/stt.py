...
async def _(event):
    ...
    arankevent = await edit_or_reply(event, "`Downloading to my local, for analysis  🙇`")
    required_file_name = await event.client.download_media(reply, Config.TEMP_DIR)
    await arankevent.edit("`Starting analysis, using IBM WatSon Speech To Text`")
    headers = {
    ...
    r = response.json()
    if "results" not in r:
        return await arankevent.edit(r["error"])
    ...
    await arankevent.edit(string_to_show)
