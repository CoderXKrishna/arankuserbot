...
async def get_curr_track(lfmbio):  # sourcery no-metrics
    ...
    while LASTFM_.LASTFMCHECK:
        try:
            ...
            user_info = (await arankub(GetFullUserRequest(LASTFM_.USER_ID))).full_user
            ...
            if (
                ...
                try:
                    ...
                    await arankub(UpdateProfileRequest(about=lfmbio))
                except AboutTooLongError:
                    short_bio = f"🎧: {LASTFM_.SONG}"
                    await arankub(UpdateProfileRequest(about=short_bio))
            if playing is None and user_info.about != DEFAULT_BIO:
                await sleep(6)
                await arankub(UpdateProfileRequest(about=DEFAULT_BIO))
                if BOTLOG and LASTFM_.LastLog:
                    await arankub.send_message(
            ...
            if BOTLOG and LASTFM_.LastLog:
                await arankub.send_message(BOTLOG_CHATID, f"Error changing bio:\n{err}")
        await sleep(2)
...
async def last_fm(lastFM):
    ...
    arankevent = await edit_or_reply(lastFM, "Processing...")
    preview = None
    playing = User(LASTFM_USERNAME, lastfm).get_now_playing()
    ...
    if preview is not None:
        await edit_or_reply(arankevent, f"{output}", parse_mode="md", link_preview=True)
    else:
        await edit_or_reply(arankevent, f"{output}", parse_mode="md")
