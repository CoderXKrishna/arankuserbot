...
async def remove_background(event):
    ...
    if event.reply_to_msg_id and not input_str:
        ...
        arankevent = await edit_or_reply(event, "`Analysing this Media...`")
        ...
        arankevent = await edit_or_reply(event, "`Removing Background of this media`")
    ...
    if "image" not in contentType:
        return await edit_delete(arankevent, f"`{response.content.decode('UTF-8')}`", 5)
    ...
    if cmd == "srmbg":
        file = (
            await Convert.to_sticker(
                arankevent, remove_bg_image, file="rmbgsticker.webp", noedits=True
            )
        )[1]
