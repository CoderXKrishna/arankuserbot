...
from . import BOTLOG, BOTLOG_CHATID, LyricsGen, arankub
...
# [----------------------------------------------------------------------]
# Errors
no_sp_vars = "Vars `SPOTIFY_CLIENT_ID` & `SPOTIFY_CLIENT_SECRET` are missing, add them first !\n\n[Follow this tutorial](https://graph.org/Steps-of-setting-Spotify-Vars-in-arankuserbot-04-24-2)"
...
async def spotify_setup(event):
    ...
    if not BOTLOG:
        return await edit_delete(
            event,
            "For authenarankion you need to set `PRIVATE_GROUP_BOT_API_ID` in heroku",
            7,
        )
    ...
    async with event.client.conversation(BOTLOG_CHATID) as conv:
        msg = await conv.send_message(
            ...
            f"your browser: {authurl.format(SPOTIFY_CLIENT_ID)} and reply this msg with the Page Url you got after giving authenarankion."
        )
    ...
    async def spotify_bio():  # sourcery no-metrics
        ...
        while SP_DATABASE.SPOTIFY_MODE:
            ...
            if r.status_code == 200:
                ...
                if received["currently_playing_type"] == "track":
                    ...
                    if save_spam("spotify", False):
                        ...
                        await arankub.send_message(BOTLOG_CHATID, string)
                ...
                if save_spam("spotify", False):
                    ...
                    await arankub.send_message(BOTLOG_CHATID, stringy)
            ...
            try:
                ...
                full = (await arankub(GetFullUserRequest(arankub.uid))).full_user
                bio = full.about
                ...
                if to_insert:
                    ...
                    if new_bio:
                        ...
                        if new_bio != bio:
                            try:
                                await arankub(UpdateProfileRequest(about=new_bio))
                    ...
                    if not new_bio and save_spam("telegram", True):
                        ...
                        await arankub.send_message(BOTLOG_CHATID, to_send)
                else:
                    if save_spam("telegram", False):
                        stringy = (
                    ...
                    if not bio or "🎶" not in bio and bio != old_bio:
                        ...
                        await arankub(UpdateProfileRequest(about=old_bio))
...
async def spotifybio(event):
    ...
    if SP_DATABASE.SPOTIFY_MODE:
        SP_DATABASE.SPOTIFY_MODE = False
        if USER_INITIAL_BIO:
            await arankub(UpdateProfileRequest(about=USER_INITIAL_BIO["bio"]))
        ...
        USER_INITIAL_BIO["bio"] = (
            (await arankub(GetFullUserRequest(arankub.uid))).full_user
        ).about or ""
...
def file_check():
    ...
    if not os.path.exists(logo):
        urllib.request.urlretrieve(
            "https://github.com/CoderXKrishna/arankUserbot-Resources/raw/master/Resources/Spotify/arank.png",
            logo,
        )
    if not os.path.exists(font_mid):
        urllib.request.urlretrieve(
            "https://github.com/CoderXKrishna/arankUserbot-Resources/blob/master/Resources/Spotify/GoogleSans-Medium.ttf?raw=true",
            font_mid,
        )
    if not os.path.exists(font_bold):
        urllib.request.urlretrieve(
            "https://github.com/CoderXKrishna/arankUserbot-Resources/blob/master/Resources/Spotify/ArialUnicodeMS.ttf?raw=true",
            font_bold,
        )
...
async def make_thumb(url, client, song, artist, now, full):
    ...
    arank = Image.open(logo, "r").resize((1024 // 5, 1024 // 5))
    thumbmask = Image.new("RGBA", (1024, 1024), 0)
    thumbmask.paste(background, (0, 0))
    thumbmask.paste(arank, (-30, 840), mask=arank)
    ...
    try:
        ...
        myphoto = urllib.request.urlretrieve(
            "https://github.com/CoderXKrishna/arankUserbot-Resources/raw/master/Resources/Spotify/Swagarank.jpg"
        )
...
async def spotify_inline_article():
    thumb = None
    media = "https://github.com/CoderXKrishna/arankUserbot-Resources/raw/master/Resources/Inline/spotify_off.png"
    if (
        ...
        buttons = [
            Button.url(
                "Tutorial",
                "https://graph.org/Steps-of-setting-Spotify-Vars-in-arankuserbot-04-24-2",
            )
        ]
    else:
        ...
        if response.status_code == 204:
            ...
            thumb = "https://github.com/CoderXKrishna/arankUserbot-Resources/raw/master/Resources/Inline/spotify_on.png"
...
async def spotify_player(event):
    ...
    arankevent = await edit_or_reply(event, "🎶 `Fetching...`")
...
async def spotify_now(event):
    ...
    if not link:
        ...
        if r.status_code == 204:
            return await edit_delete(
                arankevent, "**I'm not listening anything right now  ;)**"
            )
        received = r.json()
        if received["currently_playing_type"] == "track":
    ...
    async with event.client.conversation(chat) as conv:
        ...
        if not song.media:
            return await edit_delete(arankevent, "**Sorry unable to find the song.  ;!**")
