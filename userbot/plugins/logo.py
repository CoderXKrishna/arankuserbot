...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}logo arankUserbot",
            "{tr}logo 10;arankUserbot",
            "{tr}flogo arankUserBot",
            "{tr}flogo 5; arankUserbot",
            "{tr}slogo arankUserBot",
        ],
    },
)
async def very(event):  # sourcery no-metrics
    ...
    LOGO_BACKGROUND = loader2 = (
        ...
        or "https://github.com/CoderXKrishna/arankUserbot-Resources/raw/master/Resources/Logo/Background/black.jpg"
    )
    LOGO_FONT = loader3 = (
        gvarstatus("LOGO_FONT")
        or "https://github.com/CoderXKrishna/arankUserbot-Resources/blob/master/Resources/Logo/Fonts/Streamster.ttf?raw=true"
    )
    rcheck = random_checker(LOGO_FONT, LOGO_FONT_COLOR, LOGO_BACKGROUND)
    if rcheck:
        rjson = requests.get(
            "https://raw.githubusercontent.com/CoderXKrishna/arankUserbot-Resources/master/Resources/Logo/resources.txt"
        ).json()
    ...
    for i in range(count):
        ...
        try:
            ...
            await edit_or_reply(arankevent, f"**Bad Url:** {LOGO_BACKGROUND}\n\n{e}")
...
async def bad(event):
    ...
    string = (
        ...
        else f"https://github.com/CoderXKrishna/arankUserbot-Resources/raw/master/Resources/Logo/Background/{input_str}.jpg"
    )
...
async def pussy(event):  # sourcery no-metrics
    ...
    if cmd == "":
        source = requests.get(
            "https://github.com/CoderXKrishna/arankUserbot-Resources/tree/master/Resources/Logo/Fonts"
        )
        ...
        for i, each in enumerate(links, start=1):
            arank = os.path.splitext(each.text)[0]
            logo_font.append(arank)
            font_name += f"**{i}.**  `{arank}`\n"
        ...
        if input_str != "Random":
            ...
            string = f"https://github.com/CoderXKrishna/arankUserbot-Resources/blob/master/Resources/Logo/Fonts/{input_str}.ttf?raw=true"
    ...
    arank = re.compile(r"^\-?[1-9][0-9]*\.?[0-9]*")
    isint = re.match(arank, input_str)
