...
def verify_cond(arankarray, text):
    return any(i in text for i in arankarray)


def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"arankuserbot_{userid}_{pack}_anim"
    if is_video:
        return f"arankuserbot_{userid}_{pack}_vid"
    return f"arankuserbot_{userid}_{pack}"


def char_is_emoji(character):
    return character in arankemoji.UNICODE_EMOJI["en"]
...
async def delpack(arankevent, conv, args, packname):
    try:
        await conv.send_message("/delpack")
    except YouBlockedUserError:
        await arankub(unblock("stickers"))
        await conv.send_message("/delpack")
...
async def add_to_pack(
    ...
    while ("50" in x.message) or ("120" in x.message):
        ...
        await arankevent.edit(f"`Switching to Pack {pack} due to insufficient space`")
        await conv.send_message(packname)
        x = await conv.get_response()
...
async def kang(args):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    if message and message.media:
        ...
        if memetype == "Photo":
            arankevent = await edit_or_reply(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            ...
            if memetype == "Video Sticker":
                ...
                for attribute in attributes:
                    if isinstance(attribute, DocumentAttributeSticker):
                        if message.media.document.size > 261120:
                            ...
                            await edit_or_reply(
                                arankevent, f"`{random.choice(KANGING_STR)}`"
                            )
                        else:
                            arankevent = await edit_or_reply(
                ...
                await edit_or_reply(arankevent, f"`{random.choice(KANGING_STR)}`")
        else:
    ...
    if photo:
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await arankevent.edit("check `.info stickers`")
        ...
        if (
            ...
            await arankevent.edit("`Brewing a new Pack...`")
            async with args.client.conversation("@Stickers") as conv:
...
async def pack_kang(event):  # sourcery no-metrics
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await edit_delete(arankevent, "`Unsupported File!`")
            return
        if photo:
            ...
            if (
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, arankpackname = await newpacksticker(
                        arankevent,
                        conv,
                        cmd,
                        event,
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, arankpackname = await add_to_pack(
                        arankevent,
                        conv,
                        event,
                        packname,
            ...
            if arankpackname is None:
                return
            if arankpackname not in blablapacks:
                blablapacks.append(arankpackname)
...
async def pussyarank(event):
    ...
    packname = f"arank_{userid}_temp_pack"
    response = urllib.request.urlopen(
        urllib.request.Request(f"http://t.me/addstickers/{packname}")
    )
    htmlstr = response.read().decode("utf8").split("\n")
...
async def pic2packcmd(event):
    ...
    arankevent = await edit_or_reply(event, "__🔪Cropping and adjusting the image...__")
    try:
        ...
        emoji = "▫️"
    chat = "@Stickers"
    name = "arankUserbot_" + "".join(
        random.choice(list(string.ascii_lowercase + string.ascii_uppercase))
        for _ in range(16)
    )
    image = await Convert.to_image(
        arankevent, reply, dirct="./temp", file="stickers.png", noedits=True
    )
    if image[1] is None:
        return await edit_delete(
    ...
    newimg.save(new_img)
    new_img.seek(0)
    arankevent = await event.edit("__Making the pack.__")
    async with event.client.conversation(chat) as conv:
        ...
        for im in images:
            ...
            i += 1
            await arankevent.edit(f"__Making the pack.\nProgress: {i}/{len(images)}__")
...
async def get_pack_info(event):
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await arankevent.edit("`This is not a sticker. Reply to a sticker.`")
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
                id=stickerset_attr.stickerset.id,
...
async def cb_sticker(event):
    ...
    arankevent = await edit_or_reply(event, "`Searching sticker packs....`")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await edit_delete(arankevent, "`No results found :(.`", 5)
