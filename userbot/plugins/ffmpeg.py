...
FF_MPEG_DOWN_LOAD_MEDIA_PATH = os.path.join(
    Config.TMP_DOWNLOAD_DIRECTORY, "arankuserbot.media.ffmpeg"
)
...
async def ffmpeg_compress(event):  # sourcery skip: low-code-quality
    ...
    if not crf:
        crf = "23"
    dlpath = os.path.join(Config.TMP_DOWNLOAD_DIRECTORY, "arank.media.ffmpeg")
    if reply_message and reply_message.media:
        ...
        arankevent = await edit_or_reply(event, "`Saving the file...`")
        try:
            c_time = time.time()
            dl = io.FileIO(dlpath, "a")
            await event.client.fast_download_file(
                loarankion=reply_message.document,
                out=dl,
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, arankevent, c_time, "Trying to download")
                ),
            )
            dl.close()
        except Exception as e:
            await edit_or_reply(arankevent, f"**Error:**\n`{e}`")
        else:
            await edit_or_reply(arankevent, "`Processing...`")
            delete = True
    ...
    compress = await convert_video(
        dlpath, "./temp", crf, old["duration"], arankub, arankevent
    )
    cend = datetime.now()
    ...
    if delete:
        os.remove(dlpath)
    if not compress:
        return await edit_delete(arankevent, "**ERROR :: Unalble to Compress**")
    ...
    if cmd == "f":
        try:
            c_time = time.time()
            arankt = await event.client.send_file(
                event.chat_id,
                compress,
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, arankevent, c_time, "Trying to upload")
                ),
            )
            os.remove(compress)
        except Exception as e:
            return await edit_delete(arankevent, f"**Error : **`{e}`")
    else:
    ...
    await edit_or_reply(arankt, cap)
...
async def ff_mpeg_trim_cmd(event):
    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        if mpath and os.path.exists(mpath):
            ...
            await _arankutils.runcmd(f"cp -r {mpath} {FF_MPEG_DOWN_LOAD_MEDIA_PATH}")
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "note": "if you haven't mentioned time interval and just time then will send screenshot at that loarankion.",
...
async def ff_mpeg_trim_cmd(event):
    ...
    reply_to_id = await reply_id(event)
    arankevent = await edit_or_reply(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await edit_delete(
                arankevent, "**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
        ...
        await edit_delete(arankevent, "RTFM")
        return
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(arankevent, f"`Completed Process in {ms} seconds`", 3)
...
async def ff_mpeg_trim_cmd(event):
    ...
    if o is None:
        return await edit_delete(arankevent, "**Error : **`Can't complete the process`")
    try:
        c_time = time.time()
        await event.client.send_file(
            event.chat_id,
