...
async def carbon_api(event):
    ...
    if not query:
        return await edit_delete(arank, "No text was given")

    code = quote_plus(deEmojify(query))
    arank = await edit_or_reply(event, "`Carbonizing...\n25%`")
    ...
    driver.get(url)
    await edit_or_reply(arank, "`Be Patient...\n50%`")
    download_carbon(driver, url)

    await edit_or_reply(arank, "`Processing..\n75%`")

    await asyncio.sleep(2)
    await edit_or_reply(arank, "`Done Dana Done...\n100%`")
    file = "./carbon.png"
    await edit_or_reply(arank, "`Uploading..`")
...
async def kar1_api(event):
    """A Wrapper for carbon.now.sh"""
    arank = await edit_or_reply(event, "🔲🔲🔲🔲🔲")
    ...
    await edit_or_reply(arank, "🔳🔳🔳🔳🔳")
    file = "./carbon.png"
    await edit_or_reply(arank, "☣️Karbon1 Completed, Uploading Karbon☣️")
    await event.client.send_file(
        event.chat_id,
        file,
        force_document=True,
...
async def kar2_api(event):
    ...
    await edit_or_reply(arank, "🔘🔘🔘🔘🔘")
    file = "./carbon.png"
    await edit_or_reply(arank, "☣️Karbon2 Completed, Uploading Karbon☣️")
    await event.client.send_file(
        event.chat_id,
        file,
...
async def kar3_api(event):
    ...
    await edit_or_reply(arank, "🔵🔵🔵🔵🔵")
    file = "./carbon.png"
    await edit_or_reply(arank, "☣️Karbon3 Completed, Uploading Karbon⬆️")
    await event.client.send_file(
        event.chat_id,
        file,
...
async def kar4_api(event):
    ...
    await asyncio.sleep(1)
    file = "./carbon.png"
    await edit_or_reply(arank, "✅Karbon4 Completed, Uploading Karbon✅")
    await event.client.send_file(
        event.chat_id,
        file,
...
async def kargb_api(event):
    ...
    await edit_or_reply(arank, "⬛⬛⬛⬛⬛")
    file = "./carbon.png"
    await edit_or_reply(arank, "✅RGB Karbon Completed, Uploading Karbon✅")
    await event.client.send_file(
        event.chat_id,
        file,
