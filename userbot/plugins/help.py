...
@arankub.arank_cmd(
    ...
    info={
        "header": "To get guide for arankuserbot.",
...
async def _(event):
    ...
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of arankuserbot are :"
    )
...
async def _(event):
    ...
    if found := [i for i in sorted(list(CMD_INFO)) if cmd in i]:
        ...
        out = f"I can't find any such command `{cmd}` in arankUserbot"
