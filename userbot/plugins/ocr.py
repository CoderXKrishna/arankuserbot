...
from . import Convert, arankub, soft_deEmojify
...
async def ocr(event):
    ...
    arankevent = await edit_or_reply(event, "`Reading...`")
    ...
    if not output_file[1]:
        return await edit_delete(
            arankevent, "`Couldn't find image. Are you sure you replied to image?`"
        )
    ...
    try:
        ...
        await edit_delete(
            arankevent, "`Couldn't read it.`\n`I guess I need new glasses.`"
        )
    else:
        if cmd == "":
            await edit_or_reply(
                arankevent, f"**Here's what I could read from it:**\n\n`{ParsedText}`"
            )
        if cmd == "t":
            ...
            try:
                ...
                return await edit_delete(arankevent, "`Invalid destination language.`")
