...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": ["{tr}qpic arankUserbot.", "{tr}qpic -b arankUserbot."],
    },
)
async def q_pic(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    arankevent = await edit_or_reply(event, "__Making Quote pic....__")
    ...
    output = io.BytesIO()
    if sticker:
        output.name = "arankUserbot.Webp"
        img.save(output, "webp")
    else:
        output.name = "arankUserbot.png"
        img.save(output, "PNG")
    output.seek(0)
...
async def stickerchat(arankquotes):
    "Makes your message as sticker quote"
    reply = await arankquotes.get_reply_message()
    cmd = arankquotes.pattern_match.group(1)
    mediatype = None
    if cmd in ["rq", "q", "frq"]:
        if not reply:
            return await edit_or_reply(
                arankquotes, "`I cant quote the message . reply to a message`"
            )
    ...
    if mediatype and mediatype in ["Photo", "Round Video", "Gif"]:
        return await edit_or_reply(arankquotes, "`Replied message is not supported now`")
    arankevent = await edit_or_reply(arankquotes, "`Making quote...`")
    if cmd in ["rq", "q"]:
        try:
            user = (
                await arankquotes.client.get_entity(reply.forward.sender)
                if reply.fwd_from
                else reply.sender
            )
        except TypeError:
        ...
        user, rank = await get_user_from_event(arankquotes, secondgroup=True)
        if not user:
            return
        fetchmsg = rank
        ...
        if not fetchmsg:
            return await edit_or_reply(
                arankquotes, "`I cant quote the message . no text is given`"
            )
    res, arankmsg = await process(
        fetchmsg, user, arankquotes.client, reply, arankquotes, repliedreply
    )
    if not res:
        return
    ...
    endfi = await Convert.to_sticker(
        arankquotes, outfi, file="stickerchat.webp", noedits=True
    )
    await arankquotes.client.send_file(arankquotes.chat_id, endfi[1], reply_to=reply)
    await arankevent.delete()
    os.remove(endfi[1])
...
async def _(event):
    ...
    chat = "@QuotLyBot"
    arankevent = await edit_or_reply(event, "```Making a Quote```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if messages_id != []:
                ...
                return await edit_delete(
                    arankevent, "`I guess you have used a invalid syntax`"
                )
            response = await response
        except YouBlockedUserError:
            return await arankevent.edit("```Please unblock me (@QuotLyBot) u Nigga```")
