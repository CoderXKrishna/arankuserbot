...
async def gen_img(odi):
    ...
    arankevent = await edit_or_reply(odi, "`Processing ...`")
        ...
        if query.isnumeric():
            if int(query) in rstyles:
                ...
                return await edit_delete(
                    arankevent, f"`Style changed to {rstyles[int(query)]}.`"
                )
    ...
    await edit_or_reply(arankevent, "`Generating ai image ...`")
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": "{tr}gentxt write a paragraph on arank",
    },
)
async def gen_txt(event):
    ...
    arankevent = await edit_or_reply(event, "`Generating ai response ...`")
    if generated_text := arankai.get_response(query):
        await edit_or_reply(arankevent, generated_text)
    else:
        await edit_delete(arankevent, "`Sorry, unable to generate response`")
