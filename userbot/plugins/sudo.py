...
@arankub.arank_cmd(
    ...
    info={
        "header": "To enable or disable sudo of your arankuserbot.",
...
async def chat_blacklist(event):
    "To enable or disable sudo of your arankUserbot."
...
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "__There are no sudo users for your arankuserbot.__"
        )
    result = "**The list of sudo users for your arankuserbot are :**\n\n"
...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    if input_str[0] == "-all":
        arankevent = await edit_or_reply(event, "__Enabling all safe cmds for sudo....__")
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your arankUserbot__.\n"
                )
            else:
        ...
        for cmd in input_str:
            if cmd not in CMD_INFO:
                errors += f"`{cmd}` __There is no such command in your arankUserbot__.\n"
    ...
    result = f"__Successfully enabled __ `{len(loadcmds)}` __ for arankUserbot sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
        output += "\n**Errors:**\n" + errors
    msg = await edit_or_reply(arankevent, output)
    await event.client.reload(msg)
...
async def _(event):  # sourcery no-metrics  # sourcery skip: low-code-quality
    ...
    result = f"__Successfully disabled __ `{count}` __ for arankUserbot sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
...
arankub.loop.create_task(_init())
