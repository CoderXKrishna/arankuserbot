...
async def group_has_sedbot(group):
    if isinstance(group, types.InputPeerChannel):
        full = await arankub(functions.channels.GetFullChannelRequest(group))
    elif isinstance(group, types.InputPeerChat):
        full = await arankub(functions.messages.GetFullChatRequest(group.chat_id))
    else:
        return False
...
@arankub.on(events.NewMessage)
async def on_message(event):
    last_msgs[event.chat_id].appendleft(event.message)


@arankub.on(events.MessageEdited)
