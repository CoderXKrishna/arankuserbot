...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": "{tr}msgto @arankuserbotot just a testmessage",
    },
)
async def arankbroadcast_add(event):
...
async def arankbroadcast_add(event):
    ...
    arankinput_str = event.pattern_match.group(1)
    if not arankinput_str:
        return await edit_delete(
            event,
            "In which category should i add this chat",
            parse_mode=_format.parse_pre,
        )
    keyword = arankinput_str.lower()
...
async def arankbroadcast_list(event):
    ...
    await edit_or_reply(arankevent, finaloutput)
...
async def arankbroadcast_send(event):
    ...
    await edit_delete(arankevent, resultext)
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
...
async def arankbroadcast_remove(event):
...
async def arankbroadcast_delete(event):
    "To delete a category completely."
    arankinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(arankinput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {arankinput_str}",
            parse_mode=_format.parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(arankinput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {arankinput_str}",
            parse_mode=_format.parse_pre,
        )
    except Exception as e:
        await edit_delete(
