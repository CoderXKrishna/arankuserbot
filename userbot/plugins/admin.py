...
async def promote(event):
    ...
    if not user:
        return
    arankevent = await edit_or_reply(event, "`Promoting...`")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await arankevent.edit(NO_PERM)
    await arankevent.edit("`Promoted Successfully! Now gib Party`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#PROMOTE\
...
async def demote(event):
    ...
    if not user:
        return
    arankevent = await edit_or_reply(event, "`Demoting...`")
    ...
    try:
        ...
        return await arankevent.edit(NO_PERM)
    await arankevent.edit("`Demoted Successfully! Betterluck next time`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#DEMOTE\
...
async def _ban_person(event):
    ...
    arankevent = await edit_or_reply(event, "`Whacking the pest!`")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    arankevent = await edit_or_reply(event, "`Unbanning...`")
    try:
        ...
        await arankevent.edit("`Uh oh my unban logic broke!`")
    except Exception as e:
        await arankevent.edit(f"**Error :**\n`{e}`")
...
async def kick(event):
    ...
    if not user:
        return
    arankevent = await edit_or_reply(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await arankevent.edit(f"{NO_PERM}\n{e}")
    if reason:
        await arankevent.edit(
            f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`\nReason: {reason}"
        )
    else:
        await arankevent.edit(f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#KICK\n"
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    arankevent = await edit_or_reply(event, "`Searching recent actions .....`")
    ...
    if not flag:
        ...
        await edit_or_reply(arankevent, deleted_msg)
    else:
        main_msg = await edit_or_reply(arankevent, deleted_msg)
        for msg in adminlog:
            ruser = await event.client.get_entity(msg.old.from_id)
            _media_type = await media_type(msg.old)
            if _media_type is None:
