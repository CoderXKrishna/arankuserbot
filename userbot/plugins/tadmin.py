...
async def tmuter(event):  # sourcery no-metrics
    ...
    arankevent = await edit_or_reply(event, "`muting....`")
    user, reason = await get_user_from_event(event, arankevent)
    if not user:
        return
    if not reason:
        return await arankevent.edit("you haven't mentioned time, check `.help tmute`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    aranktime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ctime = await extract_time(arankevent, aranktime)
    if not ctime:
        return
    if user.id == event.client.uid:
        return await arankevent.edit("Sorry, I can't mute myself")
    try:
        await arankevent.client(
            EditBannedRequest(
                event.chat_id,
                user.id,
...
async def tban(event):  # sourcery no-metrics
    ...
    arankevent = await edit_or_reply(event, "`banning....`")
    user, reason = await get_user_from_event(event, arankevent)
    if not user:
        return
    if not reason:
        return await arankevent.edit("you haven't mentioned time, check `.help tban`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    aranktime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ...
    if not ctime:
        return
    if user.id == event.client.uid:
        return await arankevent.edit("Sorry, I can't ban myself")
    await arankevent.edit("`Whacking the pest!`")
    try:
        await event.client(
            EditBannedRequest(
                event.chat_id,
