...
# arankuserbot Google Drive managers  ported from Projectbish and added extra things by @mrconfused
...
async def download(
    ...
    if uri:
        ...
        if aranktorrent:
            ...
            await edit_or_reply(
                gdrive,
                "`To use torrent files or download files from link install torrentutils from` @arankplugins",
            )
...
async def upload(gdrive, service, file_path, file_name, mimeType, dir_id=None):
    ...
    body = {
        "name": file_name,
        "description": "Uploaded from Telegram using arankuserbot.",
        "mimeType": mimeType,
        "parents": dir_id,
    }
...
async def generate_credentials(gdrive):
    ...
    msg = await gdrive.respond(
        "`Go to your Private log group to authentiaranke token...`"
    )
...
async def google_drive(gdrive):  # sourcery no-metrics
    ...
    if os.path.isfile(value):
        ...
        try:
            ...
            await gdrive.edit(
                "<b>[FOLDER - UPLOAD]</b>\n\n"
                f"<b>Loarankion: </b><a href='{webViewURL}'>{folder_name}</a>\n"
...
async def g_download(event):
    ...
    file_name, arankprocess = await gdrive_download(event, arankevent, service, drive_link)
    if arankprocess is not None:
        return await edit_delete(arankevent, file_name)
    ...
    if not cmd:
        ...
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, arankevent, c_time, "Uploading...", file_name)
            ),
        )
        os.remove(file_name)
        await edit_delete(
...
async def arankshare(event):
    "To share the team drive files."
    service = await create_app(event)
    if service is False:
        return None
    input_str = event.pattern_match.group(1)
    arankevent = await edit_or_reply(event, "`Creating sharable link...`")
    await asyncio.sleep(2)
    await gshare(service, arankevent, input_str)
