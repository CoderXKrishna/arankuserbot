...
async def glitch(event):
    ...
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    arankinput = int(arankinput) if arankinput else 2
    ...
    if cmd:
        ...
        glitch_img = glitcher.glitch_image(img, arankinput, color_offset=True)
        glitch_img.save(glitched)
        await event.client.send_file(event.chat_id, glitched, reply_to=arankid)
    else:
        glitched = os.path.join("./temp", "glitched.gif")
        glitch_img = glitcher.glitch_image(img, arankinput, color_offset=True, gif=True)
        DURATION = 200
        LOOP = 0
        glitch_img[0].save(
            glitched,
        ...
        sandy = await event.client.send_file(event.chat_id, glitched, reply_to=arankid)
