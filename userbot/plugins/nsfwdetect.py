...
async def detect(event):
    ...
    arankevent = await edit_or_reply(event, "`Downloading the file to check...`")
    media = await event.client.download_media(reply)
    if not media.endswith(("png", "jpg", "webp")):
        return await edit_delete(
            event, "`Reply to any image or non animated sticker !`", 5
        )
    arankevent = await edit_or_reply(event, "`Detecting NSFW limit...`")
    r = requests.post(
    ...
    os.remove(media)
    if "status" in r.json():
        return await edit_delete(arankevent, r.json()["status"])
