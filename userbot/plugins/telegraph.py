...
async def _(event):
    ...
    if input_str in ["media", "m"]:
        ...
        await arankevent.edit(f"`Downloaded to {downloaded_file_name}`")
        ...
        ms = (end - start).seconds
        arank = f"https://graph.org/{response['path']}"
        await arankevent.edit(
            f"**link : ** [telegraph]({arank})\
                 \n**Time Taken : **`{ms} seconds.`",
            link_preview=True,
        )
...
async def ctg(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await arankub(unblock("chotamreaderbot"))
            msg_flag = await conv.send_message(urls[0])
        response = await conv.get_response()
        await event.client.send_read_acknowledge(conv.chat_id)
        if response.text.startswith(""):
            await edit_or_reply(arankevent, "Am I Dumb Or Am I Dumb?")
        else:
