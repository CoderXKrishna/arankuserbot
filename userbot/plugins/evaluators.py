...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": "{tr}exec arank stringsetup.py",
    },
)
async def _(event):
    ...
    arankevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    arankuser = await event.client.get_me()
    curruser = arankuser.username or "arankuserbot"
    uid = os.geteuid()
    if uid == 0:
...
@arankub.arank_cmd(
    ...
    info={
        ...
        "examples": "{tr}eval print('arankuserbot')",
    },
)
async def _(event):
    ...
    arankevent = await edit_or_reply(event, "`Running ...`")
