...
async def aranklst_of_files(path):
    files = []
...
def sortthings(contents, path):
    ...
    for file in contents:
        arankpath = os.path.join(path, file)
        if os.path.isfile(arankpath):
            aranksort.append(file)
    for file in contents:
        arankpath = os.path.join(path, file)
        if os.path.isdir(arankpath):
            aranksort.append(file)
    return aranksort
...
async def upload(path, event, udir_event, arankflag=None):  # sourcery no-metrics
    arankflag = arankflag or False
    ...
    if os.path.isdir(path):
        ...
        for file in Files:
            arankpath = os.path.join(path, file)
            await upload(Path(arankpath), event, udir_event)
...
async def uploadir(event):
    ...
    if os.path.isdir(path):
        ...
        UPLOAD_.uploaded = 0
        await upload(path, event, udir_event, arankflag=flag)
        end = datetime.now()
