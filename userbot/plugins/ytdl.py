...
async def ytdl_down(event, opts, url):
    ...
    try:
        ...
        await event.edit(
            "`Video is not available from your geographic loarankion due to geographic restrictions imposed by a website.`"
        )
...
async def download_audio(event):  # sourcery skip: low-code-quality
    ...
    arankevent = await edit_or_reply(event, "`Preparing to download...`")
    ...
    for url in urls:
        try:
            ...
            vid_data = {"title": url, "uploader": "arankuserbot", "formats": []}
        startTime = time()
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await edit_delete(arankevent, "__Unable to upload file__")
...
async def download_video(event):
    ...
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(arankevent, video_opts, url)
        if ytdl_down is None:
            return
        try:
            f = pathlib.Path("arank_ytv.mp4")
            arankthumb = pathlib.Path("arank_ytv.jpg")
            if not os.path.exists(arankthumb):
                arankthumb = pathlib.Path("arank_ytv.webp")
            if not os.path.exists(arankthumb):
                arankthumb = None
            await arankevent.edit(
                f"`Preparing to upload video:`\
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(
                        d, t, arankevent, c_time, "Upload :", file_name=ytdl_data["title"]
                    )
                ),
            )
            ul.close()
