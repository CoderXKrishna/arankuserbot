...
from ..helpers.utils import _arankutils, _format, parse_pre, reply_id
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    arankevent = await edit_or_reply(event, "__Making round spin video wait a sec.....__")
    ...
    if final[1] is None:
        return await edit_delete(arankevent, "__Unable to make spin gif.__")
...
async def video_arankfile(event):  # sourcery no-metrics
    ...
    flag = True
    arankevent = await edit_or_reply(event, "`Converting to round format..........`")
    arankfile = await reply.download_media(file="./temp/")
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not arankfile.endswith((".webp")):
            if arankfile.endswith((".tgs")):
                await Convert.to_gif(event, arankfile, file="circle.mp4", noedits=True)
                arankfile = "./temp/circle.mp4"
            media_info = MediaInfo.parse(arankfile)
            aspect_ratio = 1
            for track in media_info.tracks:
                if track.track_type == "Video":
                    aspect_ratio = track.display_aspect_ratio
            ...
            if aspect_ratio != 1:
                crop_by = min(height, width)
                await _arankutils.runcmd(
                    f'ffmpeg -i {arankfile} -vf "crop={crop_by}:{crop_by}" {PATH}'
                )
            else:
                copyfile(arankfile, PATH)
            if str(arankfile) != str(PATH):
                os.remove(arankfile)
            try:
                arankthumb = await reply.download_media(thumb=-1)
            except Exception as e:
        ...
        try:
            ...
            arankthumb = os.path.join("./temp", "thumb.jpg")
            await thumb_from_audio(arankfile, arankthumb)
        if arankthumb is not None and not os.path.exists(arankthumb):
            arankthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, arankthumb)
        if (
            arankthumb is not None
            and not os.path.exists(arankthumb)
            and os.path.exists(thumb_loc)
        ):
            flag = False
            arankthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, arankthumb)
        if arankthumb is not None and os.path.exists(arankthumb):
            await _arankutils.runcmd(
                f"""ffmpeg -loop 1 -i {arankthumb} -i {arankfile} -c:v libx264 -tune stillimage -c:a aac -b:a 192k -vf \"scale=\'iw-mod (iw,2)\':\'ih-mod(ih,2)\',format=yuv420p\" -shortest -movflags +faststart {PATH}"""
            )
            os.remove(arankfile)
        else:
            os.remove(arankfile)
            return await edit_delete(
                arankevent, "`No thumb found to make it video note`", 5
            )
    if mediatype in [
        "Voice",
        "Audio",
        ...
        "Video",
        "Sticker",
    ] and not arankfile.endswith((".webp")):
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, arankevent, c_time, "Uploading....")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
                ...
                force_file=False,
                thumb=await event.client.upload_file(arankthumb) if arankthumb else None,
            )
            sandy = await event.client.send_file(
                event.chat_id,
                media,
    ...
    img.save(im)
    im.seek(0)
    await event.client.send_file(event.chat_id, im, reply_to=arankid)
    await arankevent.delete()
...
async def on_file_to_photo(event):
    ...
    arankt = await edit_or_reply(event, "`Converting.....`")
...
async def _(event):  # sourcery no-metrics
    ...
    arankreply = await event.get_reply_message()
    memetype = await meme_type(arankreply)
    if memetype == "Gif":
    ...
    reply_to_id = await reply_id(event)
    arankfile = await event.client.download_media(arankreply)
    final = await Convert.to_gif(event, arankfile, file="animation.mp4", noedits=True)
    arankgif = final[1]
    if arankgif is None:
        return await edit_delete(arankevent, "`Sorry couldn't convert the media to gif.`")
    sandy = await event.client.send_file(
        event.chat_id,
        arankgif,
    ...
    await unsavegif(event, sandy)
    await arankevent.delete()
    for files in (arankgif, arankfile):
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    arankevent = await edit_or_reply(event, "__🎞 Making Gif from the replied media...__")
    ...
    try:
        ...
        return await edit_delete(arankevent, f"**Error**\n__{e}__")
    output = io.BytesIO()
    output.name = "Output.gif"
    ...
    if output[0] is None:
        await edit_delete(
            arankevent, "__There was some error in the media. I can't format it to gif.__"
        )
