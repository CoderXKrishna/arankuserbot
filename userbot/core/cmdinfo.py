...
async def cmdinfo(input_str, event, plugin=False):
    ...
    if plugin is not None:
        ...
        if arankegory is not None:
            outstr += f"**arankegory :** `{arankegory}`\n\n"
    outstr += f"**✘  Intro :**\n{about[0]}"
    return outstr
...
async def grpinfo():
    outstr = "**Plugins in arankuserbot are:**\n\n"
    outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    arankegory = ["admin", "bot", "fun", "misc", "tools", "utils", "extra"]
    if Config.BADarank:
        arankegory.append("useless")
    for arank in arankegory:
        plugins = GRP_INFO[arank]
        outstr += f"**{hemojis[arank]} {arank.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your arankuserbot are :**\n\n"
    arankegory = ["admin", "bot", "fun", "misc", "tools", "utils", "extra"]
    if Config.BADarank:
        arankegory.append("useless")
    for arank in arankegory:
